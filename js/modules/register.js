...
function registerShow() { // показать регистрация
    $('.register').removeClass("not-active");
    $('a[data-section=register]').addClass("colorlib-active");
}

function registerHide() { // спрятать блок регистрации
...
function giveInputRegister() {
    $(".btn-register").on("click", function (e) { // по нажатию на кнопку регистрация в форме
        e.preventDefault();
        let $obj = {};
        $(".register-form").find("input").not(".form-check-input").each((sd, el) => { // забираем данные из формы в $obj
        ...
        $.ajax({
            ...
            success: function ($response) {
                if (!$response.status) { // если валидациия прошла успешно, скрываем регистрацию, показываем посты. Иначе должны вывести правильно валидацию в форме
                    registerHide();
                    getFullPost();
                    ...
                    Object.keys($response).map(function ($elem) { // для каждого элемента вывода work_register.php делаем следующеее
                        ...
                        if ($elem.includes("valid_")) { // если ключ включает в себя valid_ тогда
                            if ($response[$elem]) { // если элемент не false тогда
